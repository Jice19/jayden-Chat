  ...
  <div class="flex h-screen w-full bg-[var(--color-background)] overflow-hidden">
    ...
    <div class="w-64 bg-[var(--color-surface)] border-r border-[var(--color-border)] flex flex-col flex-shrink-0">
      ...
      <div class="flex-1 overflow-y-auto p-2 space-y-1">
        <div 
            ...
            :class="[
              currentSessionId === session.id 
                ? 'bg-[var(--color-hover)] text-blue-600 border-[var(--color-border)]' 
                : 'text-[var(--color-text-secondary)] hover:bg-[var(--color-hover)] hover:border-[var(--color-border)]'
            ]"
          >
            ...
            <button 
              ...
              class="ml-2 p-1 rounded-full hover:bg-[var(--color-border)] text-[var(--color-text-disabled)] hover:text-red-500 opacity-0 group-hover:opacity-100 transition-opacity"
              title="删除会话"
            >
        ...
        <div v-if="sessionList.length === 0" class="text-center py-8">
           <p class="text-[var(--color-text-disabled)] text-sm">暂无历史会话</p>
        </div>
      </div>
    ...
    <div class="flex-1 flex flex-col h-full relative">
        ...
        <div class="flex-1 p-4 scroll-smooth" :class="{ 'pt-10': isLoadingChat }">
          <div v-if="chatList.length === 0 && !isLoadingChat" class="h-full flex flex-col items-center justify-center text-[var(--color-text-disabled)]">
        ...
        <div class="bg-[var(--color-surface)] border-t border-[var(--color-border)] p-4">
          <div class="max-w-4xl mx-auto w-full">
            <div class="relative">
              <textarea 
                v-model="inputText"
                class="w-full bg-[var(--color-background)] text-[var(--color-text-primary)] border border-[var(--color-border)] rounded-xl p-3 pr-24 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent resize-none shadow-sm"
            ...
            <div class="mt-2 flex justify-between items-center text-xs text-[var(--color-text-disabled)] px-1">
